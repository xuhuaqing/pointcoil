...
void 0 == SWFUpload && (SWFUpload = function (settings) {
    ...
    try {
        this.customSettings = {}, this.settings = settings, this.eventQueue = [], this.movieName = "SWFUpload_" + SWFUpload.movieCount++, this.movieElement = null, SWFUpload.instances[this.movieName] = this, this.initSettings(), this.loadFlash(), this.displayDebugInfo()
    } catch (ex) {
        throw delete SWFUpload.instances[this.movieName], ex
    }
}, SWFUpload.instances = {}, SWFUpload.movieCount = 0, SWFUpload.version = "2.2.0 Beta 2", SWFUpload.QUEUE_ERROR = {
    ...
    this.ensureDefault = function (settingName, defaultValue) {
        this.settings[settingName] = void 0 == this.settings[settingName] ? defaultValue : this.settings[settingName]
    ...
    "" !== this.settings.button_placeholder_id ? this.replaceWithFlash() : this.appendFlash()
    ...
    if (targetElement = document.getElementsByTagName("body")[0], void 0 == targetElement) throw"Could not find the 'body' element.";
    container = document.createElement("div"), container.style.width = "1px", container.style.height = "1px", container.style.overflow = "hidden", targetElement.appendChild(container), container.innerHTML = this.getFlashHTML()
}, SWFUpload.prototype.replaceWithFlash = function () {
    var targetElement, tempParent;
    if (null !== document.getElementById(this.movieName)) throw"ID " + this.movieName + " is already in use. The Flash Object could not be added";
    if (targetElement = document.getElementById(this.settings.button_placeholder_id), void 0 == targetElement) throw"Could not find the placeholder element.";
    tempParent = document.createElement("div"), tempParent.innerHTML = this.getFlashHTML(), targetElement.parentNode.replaceChild(tempParent.firstChild, targetElement)
    ...
    var paramString = this.buildParamString(), httpSuccessString = this.settings.http_success.join(",");
    ...
    var postParams = this.settings.post_params, paramStringPairs = [];
    if ("object" == typeof postParams) for (var name in postParams) postParams.hasOwnProperty(name) && paramStringPairs.push(encodeURIComponent(name.toString()) + "=" + encodeURIComponent(postParams[name].toString()));
    ...
    try {
        ...
        if (void 0 != movieElement && void 0 != movieElement.parentNode && "function" == typeof movieElement.parentNode.removeChild) {
            var container = movieElement.parentNode;
            void 0 != container && (container.removeChild(movieElement), void 0 != container.parentNode && "function" == typeof container.parentNode.removeChild && container.parentNode.removeChild(container))
        }
        return SWFUpload.instances[this.movieName] = null, delete SWFUpload.instances[this.movieName], delete this.movieElement, delete this.settings, delete this.customSettings, delete this.eventQueue, delete this.movieName, delete window[this.movieName], !0
    } catch (ex1) {
        return !1
    }
    ...
    return void 0 == value ? this.settings[name] = default_value : this.settings[name] = value
}, SWFUpload.prototype.getSetting = function (name) {
    return void 0 != this.settings[name] ? this.settings[name] : ""
}, SWFUpload.prototype.callFlash = function (functionName, argumentArray) {
    argumentArray = argumentArray || [];
    var returnValue, movieElement = this.getMovieElement();
    if ("function" == typeof movieElement[functionName]) {
        if (0 === argumentArray.length) returnValue = movieElement[functionName](); else if (1 === argumentArray.length) returnValue = movieElement[functionName](argumentArray[0]); else if (2 === argumentArray.length) returnValue = movieElement[functionName](argumentArray[0], argumentArray[1]); else {
            if (3 !== argumentArray.length) throw"Too many arguments";
    ...
    this.settings.upload_url = url.toString(), this.callFlash("SetUploadURL", [url])
    ...
    void 0 == argumentArray ? argumentArray = [] : argumentArray instanceof Array || (argumentArray = [argumentArray]);
    var self = this;
    if ("function" == typeof this.settings[handlerName]) this.eventQueue.push(function () {
        this.settings[handlerName].apply(this, argumentArray)
    }), setTimeout(function () {
        self.executeNextEvent()
    }, 0); else if (null !== this.settings[handlerName]) throw"Event handler " + handlerName + " is unknown or is not a function"
}, SWFUpload.prototype.executeNextEvent = function () {
    var f = this.eventQueue ? this.eventQueue.shift() : null;
    "function" == typeof f && f.apply(this)
}, SWFUpload.prototype.unescapeFilePostParams = function (file) {
    var uk, reg = /[$]([0-9a-f]{4})/i, unescapedPost = {};
    if (void 0 != file) {
        for (var k in file.post) if (file.post.hasOwnProperty(k)) {
            uk = k;
            for (var match; null !== (match = reg.exec(uk));) uk = uk.replace(match[0], String.fromCharCode(parseInt("0x" + match[1], 16)));
    ...
    if ("function" != typeof movieElement.StartUpload) throw"ExternalInterface methods failed to initialize.";
    void 0 == window[this.movieName] && (window[this.movieName] = movieElement), this.queueEvent("swfupload_loaded_handler")
    ...
    var returnValue;
    if ("function" == typeof this.settings.upload_start_handler) file = this.unescapeFilePostParams(file), returnValue = this.settings.upload_start_handler.call(this, file); else if (void 0 != this.settings.upload_start_handler) throw"upload_start_handler must be a function";
    ...
    file = this.unescapeFilePostParams(file), this.queueEvent("upload_success_handler", [file, serverData])
    ...
    if (this.settings.debug) {
        ...
        if ("object" == typeof message && "string" == typeof message.name && "string" == typeof message.message) {
            for (var key in message) message.hasOwnProperty(key) && exceptionValues.push(key + ": " + message[key]);
